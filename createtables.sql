...
CREATE TABLE THONGTINDATCOC (
    MaTTDC VARCHAR(10) NOT NULL PRIMARY KEY DEFAULT '0',
    MaKH VARCHAR(10),
    MaXe VARCHAR(10),
    MaNV VARCHAR(10),
    ...
    CONSTRAINT `fk THONGTINDATCOC.MaKH to KHACHHANG.MaKH` FOREIGN KEY (MaKH)
        REFERENCES KHACHHANG (MaKH),
    CONSTRAINT `fk THONGTINDATCOC.MaXe to XE.MaXe` FOREIGN KEY (MaXe)
        REFERENCES XE (MaXe),
    CONSTRAINT `fk THONGTINDATCOC.MaNV to NHANVIEN.MaNV` FOREIGN KEY (MaNV)
        REFERENCES NHANVIEN (MaNV),
    CONSTRAINT `fk THONGTINDATCOC.MaKM to KHUYENMAI.MaKM` FOREIGN KEY (MaKM)
        REFERENCES KHUYENMAI (MaKM)
);


CREATE TABLE HOADON (
    MaHD VARCHAR(10) NOT NULL PRIMARY KEY DEFAULT '0',
    MaTTDC varchar(10),
    MaXe varchar(10),
    MaNV varchar(10),
    MaKH varchar(10),
    ...
    PTTT VARCHAR(50) NOT NULL,
    CONSTRAINT `fk HOADON.MaTTDC to THONGTINDATCOC.MaTTDC` FOREIGN KEY (MaTTDC)
        REFERENCES THONGTINDATCOC (MaTTDC),
    CONSTRAINT `fk HOADON.MaXe to XE.MaXe` FOREIGN KEY (MaXe)
        REFERENCES XE (MaXe),
    CONSTRAINT `fk HOADON.MaNV to NHANVIEN.MaNV` FOREIGN KEY (MaNV)
        REFERENCES NHANVIEN (MaNV),
    CONSTRAINT `fk HOADON.MaKH to KHACHHANG.MaKH` FOREIGN KEY (MaKH)
        REFERENCES KHACHHANG (MaKH),
    CONSTRAINT `fk HOADON.MaKM to KHUYENMAI.MaKM` FOREIGN KEY (MaKM)
        REFERENCES KHUYENMAI (MaKM)
);
...
-- Trigger for THONGTINDATCOC
DELIMITER $$
CREATE TRIGGER `tg_THONGTINDATCOC_insert`
BEFORE INSERT ON `QuanLyShowroomOto`.`THONGTINDATCOC`
FOR EACH ROW
BEGIN
  SET NEW.`MaTTDC` = CONCAT('TTDC', LPAD((SELECT IFNULL(MAX(CAST(SUBSTRING(`MaTTDC`, 5) AS SIGNED)), 0) + 1 FROM `QuanLyShowroomOto`.`THONGTINDATCOC`), 4, '0'));
END$$
DELIMITER ;
...
BEFORE INSERT ON `QuanLyShowroomOto`.`HOADON`
FOR EACH ROW
BEGIN
  SET NEW.`MaHD` = CONCAT('HD', LPAD((SELECT IFNULL(MAX(CAST(SUBSTRING(`MaHD`, 3) AS SIGNED)), 0) + 1 FROM `QuanLyShowroomOto`.`HOADON`), 4, '0'));
END$$
DELIMITER ;
